...
<body>
	...
	$(document).ready(function() {
	            ...
	            success: function(response) {
					...
					response.roomList.forEach(function(item) {
					    ...
					    if (item.roomStatus === 'DIRTY') {
					        row += '<td><a href="#" onclick="sendData(' + item.roomNo + ')" class="btn btn-primary">'+item.roomStatus+'</a></td>';
	    ...
	    function sendData(roomNo) {
	        $.ajax({
	            url: '/topaz/groupware/room/roomList/state',
	            type: 'POST',
	            data: {
	                roomNo: roomNo
	            },
	            success: function(response) {
	                if (response.success) {
	                    loadPage(1); // 방 목록 새로 고침
	                } else {
	                    alert("상태 업데이트에 실패했습니다."); // 실패 처리
	                }
	            },
	            error: function(xhr, status, error) {
	                console.error('오류 발생:', error);
	                alert("오류가 발생했습니다."); // 오류 처리
	            }
	        });
	    }
